#Define times of analysis (choose)
around=c(-50,-45,-15,0,15,30,45,50)
around=c(-60,0,60)
around=c(0)
...
full_evolution=ggplot(evolution.measures,aes(x=PC1,y=PC2,color=(reference)))+
  ...
  scale_color_gradient(low="blue", high="red")+facet_wrap(~RegionName,ncol = 7,nrow = 8)+xlim(-3,3)

selected.states=evolution.measures%>%filter(RegionName%in%estados)
selected.states%>%filter(RegionName=='New York')
...
selected_evolution=ggplot(data=selected.states,aes(x=PC1,y=PC2,color=(reference)))+
  ...
  labs(color='Days since peak')+theme(legend.position="bottom",legend.spacing.x = unit(0.5, 'cm'))+guides(fill = guide_legend(label.position = "bottom"))+xlim(-10,10)+ylim(-10,10)
  selected_evolution
